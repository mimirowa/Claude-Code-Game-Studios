#!/bin/bash
# Codex PreCompact hook: persist a recovery snapshot before compression.
# PreCompact stdout is not model-visible, so write the snapshot to ignored
# session state that SessionStart can load after compaction.

SESSION_STATE_DIR="production/session-state"
mkdir -p "$SESSION_STATE_DIR" 2>/dev/null
SNAPSHOT_FILE="$SESSION_STATE_DIR/compact-snapshot.md"
exec > "$SNAPSHOT_FILE"
...
echo "After compaction, read $STATE_FILE and $SNAPSHOT_FILE to recover context."
